...
const updateUser = async (data) =>{
    try {
        ...
        await dbCon.run('UPDATE users SET email = (?), firstname = (?), lastname = (?), accountType = (?) WHERE id = ?', [data.email, data.firstname, data.lastname, data.accountType, data.id]);
    }
    catch (error) {
...
const getUsers = async () => {
    try {
        const dbCon = await dbPromise;
        const users = await dbCon.all('SELECT id, email, firstname, lastname, accountType FROM users ORDER BY email ASC');
        return users;
    }
    catch (error) {
...
const uppanswer = async (data) =>{
    try {
        const dbCon = await dbPromise;
        await dbCon.run('UPDATE answer SET answerText=(?), questId=(?) WHERE id=(?)', [data.answerText, data.questId, data.id]);
    }
    catch (error) {
        console.log(error)
    }
...
const loginUser = async (data) => {4
    ...
    const match = await bcrypt.compareSync(data.password, hashPassword['password']);
        ...
        if (match)
        {
            const user = await dbCon.get('SELECT email, firstname, lastname, accountType FROM users WHERE email = ?', [data.email]);
            return user; 
        }
        else
        {
